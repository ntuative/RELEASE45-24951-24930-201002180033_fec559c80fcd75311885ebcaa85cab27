package com.sulake.habbo.navigator.roomsettings
{
   ...
   public class RoomSettingsCtrl
   {
      ...
      private var var_1247:ICheckBoxWindow;
      ...
      private function populateForm() : void
      {
         ...
         var_1242.setText(_loc1_.description);
         var_1004.setText("");
         var_1246.setText("");
         ...
         switch(_loc1_.doorMode)
         {
            case RoomSettingsData.const_142:
               ...
               break;
            case RoomSettingsData.const_99:
         ...
         changePasswordField(_loc1_.doorMode == RoomSettingsData.const_99);
         Logger.log("CATEGORY ID: " + _loc1_.categoryId);
         setCategorySelection(_loc1_.categoryId);
         refreshMaxVisitors(_loc1_);
         setTag(var_744,_loc1_.tags[0]);
         setTag(var_746,_loc1_.tags[1]);
         if(var_1244)
         {
            if(_loc1_.allowPets)
            {
               this.var_1244.select();
            }
            else
            {
               this.var_1244.unselect();
            }
         }
         if(var_1241)
         {
      ...
      {
         return IWindowContainer(_navigator.windowManager.createWindow(param1,"",HabboWindowType.const_38,HabboWindowStyle.const_20,HabboWindowParam.const_50,new Rectangle(0,0,100,20)));
      }
      ...
      public function refresh(param1:IWindowContainer) : void
      {
         ...
         var_2089.visible = var_483;
         var_2090.visible = !var_483;
         if(var_483)
         {
         ...
         {
            populateForm();
            var_1007.visible = true;
            var_1245.visible = true;
            var_1243.visible = _inRoom;
            _navigator.thumbRenderer.refreshThumbnail(IWindowContainer(var_1243.findChildByName("picframe")),_navigator.data.enteredGuestRoom.thumbnail,false);
            var_1007.height = Util.getLowestPoint(this.var_1007) + 4;
         }
         var_1478.text = !!var_483 ? "${navigator.roomsettings.tobasicsettings}" : "${navigator.roomsettings.toadvancedsettings}";
         Util.moveChildrenToColumn(_window,[var_1477.name,var_1243.name,var_1007.name,var_1008.name,var_1245.name,var_1480.name],0,0);
      ...
      private function onSaveButtonClick(param1:WindowMouseEvent) : void
      {
         ...
         _loc2_.description = var_1242.getText();
         ...
         switch(_loc4_.name)
         {
            ...
            case "doormode_password":
               _loc2_.doorMode = RoomSettingsFlatInfo.const_99;
               break;
            default:
               _loc2_.doorMode = RoomSettingsFlatInfo.const_394;
         }
         if(_loc2_.doorMode == RoomSettingsFlatInfo.const_99)
         {
            _loc8_ = var_1004.getText();
            _loc9_ = var_1246.getText();
            if(_loc8_ != _loc9_)
            {
               this.var_1004.clearErrors();
               this.var_1246.displayError("${navigator.roomsettings.invalidconfirm}");
               return;
            }
            if(_loc8_ != "")
            {
         ...
         _loc2_.allowPets = var_1244.isSelected;
         _loc2_.allowFoodConsume = var_1241.isSelected;
         _loc2_.allowWalkThrough = var_1247.isSelected;
         _loc2_.tags = new Array();
         addTag(var_744,_loc2_.tags);
         addTag(var_746,_loc2_.tags);
         this.clearErrors();
         this.var_1005 = _loc2_.roomId;
      ...
      {
         if(param1 != this.var_391 || var_1005 < 1)
         {
            return;
         }
         close();
      ...
      public function onRoomSettingsSaveError(param1:int, param2:int, param3:String) : void
      {
         ...
         var_1005 = 0;
         if(param2 == RoomSettingsSaveErrorMessageParser.const_1254)
         {
            this.var_818.displayError("${navigator.roomsettings.roomnameismandatory}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1155)
         {
         ...
         {
            setTagError(this.var_744,param3,"${navigator.roomsettings.unacceptablewords}");
            setTagError(this.var_746,param3,"${navigator.roomsettings.unacceptablewords}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1324)
         {
            setTagError(this.var_744,param3,"${navigator.roomsettings.nonuserchoosabletag}");
            setTagError(this.var_746,param3,"${navigator.roomsettings.nonuserchoosabletag}");
         }
      ...
      private function prepareWindow(param1:IWindowContainer) : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_558,onDoorModePasswordSelect);
         _loc2_.addEventListener(WindowEvent.const_896,onDoorModePasswordUnselect);
         ...
         this.var_1242 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("description")),255);
         this.var_744 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("tag1")),30);
         this.var_746 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("tag2")),30);
         this.var_1004 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password")),30);
         this.var_1246 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password_confirm")),30);
         this.var_1244 = ICheckBoxWindow(_window.findChildByName("allow_pets_checkbox"));
         this.var_1241 = ICheckBoxWindow(_window.findChildByName("allow_foodconsume_checkbox"));
         this.var_1247 = ICheckBoxWindow(_window.findChildByName("allow_walk_through_checkbox"));
         var_1477 = IWindowContainer(_window.findChildByName("header_container"));
         var_1243 = IWindowContainer(_window.findChildByName("thumbnail_container"));
         var_1007 = IWindowContainer(_window.findChildByName("basic_settings_container"));
         ...
         var_1006 = IWindowContainer(_window.findChildByName("password_container"));
         var_2090 = ITextWindow(_window.findChildByName("basic_caption"));
         var_2089 = ITextWindow(_window.findChildByName("advanced_caption"));
         var_1478 = ITextWindow(_window.findChildByName("switch_view_text"));
         var_1478.procedure = onSwitchView;
         var_1006.visible = false;
         if(!_inRoom)
         {
