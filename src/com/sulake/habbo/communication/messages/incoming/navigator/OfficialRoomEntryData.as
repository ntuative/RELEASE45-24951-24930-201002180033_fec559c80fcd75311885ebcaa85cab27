package com.sulake.habbo.communication.messages.incoming.navigator
{
   import com.sulake.core.communication.messages.IMessageDataWrapper;
   import com.sulake.core.runtime.IDisposable;
   
   public class OfficialRoomEntryData implements IDisposable
   {
      
      public static const const_1319:int = 4;
      
      public static const const_689:int = 3;
      
      public static const const_608:int = 2;
      
      public static const const_838:int = 1;
       
      
      private var var_2029:String;
      
      private var _disposed:Boolean;
      
      private var var_1767:int;
      
      private var var_2031:Boolean;
      
      private var var_860:String;
      
      private var var_798:PublicRoomData;
      
      private var var_2030:int;
      
      private var _index:int;
      
      private var var_2027:String;
      
      private var _type:int;
      
      private var var_1683:String;
      
      private var var_799:GuestRoomData;
      
      private var var_2028:String;
      
      private var _open:Boolean;
      
      public function OfficialRoomEntryData(param1:IMessageDataWrapper)
      {
         super();
         _index = param1.readInteger();
         var_2029 = param1.readString();
         var_2028 = param1.readString();
         var_2031 = param1.readInteger() == 1;
         var_2027 = param1.readString();
         var_860 = param1.readString();
         var_2030 = param1.readInteger();
         var_1767 = param1.readInteger();
         _type = param1.readInteger();
         if(_type == const_838)
         {
            var_1683 = param1.readString();
         }
         else if(_type == const_689)
         {
            var_798 = new PublicRoomData(param1);
         }
         else if(_type == const_608)
         {
            var_799 = new GuestRoomData(param1);
         }
      }
      
      public function get folderId() : int
      {
         return var_2030;
      }
      
      public function get popupCaption() : String
      {
         return var_2029;
      }
      
      public function get userCount() : int
      {
         return var_1767;
      }
      
      public function get open() : Boolean
      {
         return _open;
      }
      
      public function get showDetails() : Boolean
      {
         return var_2031;
      }
      
      public function get maxUsers() : int
      {
         if(this.type == const_838)
         {
            return 0;
         }
         if(this.type == const_608)
         {
            return this.var_799.maxUserCount;
         }
         if(this.type == const_689)
         {
            return this.var_798.maxUsers;
         }
         return 0;
      }
      
      public function get popupDesc() : String
      {
         return var_2028;
      }
      
      public function dispose() : void
      {
         if(_disposed)
         {
            return;
         }
         _disposed = true;
         if(this.var_799 != null)
         {
            this.var_799.dispose();
            this.var_799 = null;
         }
         if(this.var_798 != null)
         {
            this.var_798.dispose();
            this.var_798 = null;
         }
      }
      
      public function get index() : int
      {
         return _index;
      }
      
      public function get guestRoomData() : GuestRoomData
      {
         return var_799;
      }
      
      public function get disposed() : Boolean
      {
         return _disposed;
      }
      
      public function get picText() : String
      {
         return var_2027;
      }
      
      public function get publicRoomData() : PublicRoomData
      {
         return var_798;
      }
      
      public function get picRef() : String
      {
         return var_860;
      }
      
      public function get type() : int
      {
         return _type;
      }
      
      public function get tag() : String
      {
         return var_1683;
      }
      
      public function toggleOpen() : void
      {
         _open = !_open;
      }
   }
}
