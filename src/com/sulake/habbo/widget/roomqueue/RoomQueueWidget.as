package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(var_1383 == RoomWidgetRoomQueueUpdateEvent.const_306)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_703);
         }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            var_1383 = param1.type;
            var_939 = false;
            var_940 = param1.position;
         }
         else
         {
            var_939 = true;
         }
         var_1714 = param1.hasHabboClub;
         var_1384 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",var_940.toString());
         localizations.registerParameter("room.queue.position.hc","position",var_940.toString());
         localizations.registerParameter("room.queue.spectator.position","position",var_940.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",var_940.toString());
         showInterface();
      }
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_457,onQueueStatus);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_457,onQueueStatus);
      }
      ...
      {
         messageListener.processWidgetMessage(new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_690));
      }
         ...
         if(_loc1_ != null && _loc2_ != null && _loc3_ != null)
         {
            switch(var_1383)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_306:
                  _loc1_.caption = !!var_1384 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = var_939;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_457:
                  _loc1_.caption = !!var_1384 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = var_939;
         }
         var _loc4_:IWindowContainer = _window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !var_1714;
         }
