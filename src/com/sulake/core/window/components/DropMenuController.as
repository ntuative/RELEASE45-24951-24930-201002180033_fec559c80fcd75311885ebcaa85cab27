package com.sulake.core.window.components
{
   ...
   public class DropMenuController extends InteractiveController implements IDropMenuWindow
   {
      ...
      private function getSubMenu() : DropMenuController
      {
         ...
         {
            var_114 = context.create(name + "::submenu","",WindowType.const_380,var_73,0 | 0,_loc1_,subMenuEventProc,null,0,null,[WindowController.EXCLUDE_CHILD_TAG]) as DropMenuController;
         }
         else
         {
      ...
      {
         if(param1.type == WindowMouseEvent.const_79)
         {
            if(param2 is IDropMenuItemWindow)
            {
               if(param2.testStateFlag(WindowState.const_57))
               {
         ...
         else if(param1.type == WindowEvent.const_847)
         {
            if(param2 == var_114)
            {
               closeMenuView();
      ...
      protected function populateMenuItemContainer(param1:Array, param2:DropMenuController, param3:Function) : void
      {
         ...
         while(_loc9_ < _loc5_)
         {
            _loc6_ = context.create(name + "::menu_item_" + _loc9_,param1[_loc9_],WindowType.const_363,var_73,0 | 0 | 0 | 0 | 0,null,param3,null,_loc9_,null,["_EXCLUDE"]) as DropMenuItemController;
         ...
         var _loc10_:IWindow = context.create(name + "::padding","",WindowType.const_300,var_73,0 | 0 | 0 | 0,new Rectangle(0,0,1,2),null,null,0,null,["_EXCLUDE"]);
         ...
         if(var_447 > -1 && _loc5_)
         {
            _loc4_.getListItemAt(var_447).setStateFlag(WindowState.const_29,true);
         }
      }
      ...
      protected function openMenuView() : void
      {
         if(!var_527)
         {
            if(open())
            {
               if(false)
               {
                  populateMenuItemContainer(var_446,getSubMenu(),subMenuEventProc);
                  var_527 = true;
               }
            }
         }
      ...
      protected function closeMenuView() : void
      {
         if(close())
         {
            ...
            var_527 = false;
            getTitleText().text = var_447 < var_446.length && var_447 > -1 ? "null" : caption;
         }
      }
   }
}
